...
/*
    ...
    ESP_PWR_LVL_N12 -12 dBm Lowest power, minimal range, minimal battery use
    ESP_PWR_LVL_N9  -9 dBm  Low power
    ESP_PWR_LVL_N6  -6 dBm  Low-medium
    ESP_PWR_LVL_N3  -3 dBm  Medium-low
    ESP_PWR_LVL_N0  0 dBm Default medium
    ESP_PWR_LVL_P3  +3 dBm  Medium-high
    ESP_PWR_LVL_P6  +6 dBm  High
    ESP_PWR_LVL_P9  +9 dBm  Very high, max typical
    ESP_PWR_LVL_P12 +12 dBm Maximum power, max range, more battery // Paul - this one not working?
*/
...
// Paul:
// I noticed the setBeacon() function was fixing the
// tx power value in the response frame to -12, regardless
// of what the BLE device was set to. This function should
// return the actual tx power to insert into the frame.
int8_t getTxPowerDBM() {
  ...
  switch (TX_POWER_LEVEL) {
    ...
    //case ESP_PWR_LVL_P12: return 12; // Paul: not implemented?
...
void setBeacon() {
  ...
  // Paul: original, not sure why it is fixed here?
  ret_data[12] = 0xF4; // Beacons TX power at 0m
  ret_data[12] = (uint8_t)getTxPowerDBM();// signed db
...
void setup()
{
  ...
  // Paul: we won't sleep the device, just keep
  // advertising continuously
  // Maybe useful for an intermittent device?
  //  delay(10000);
  //  pAdvertising->stop();
